    ...
    public class AppCommon
    {
        ...
        private Doc MakePdfData6(string type, DataMitumori.T_MitumoriDataTable dt, XlsxCreator xlsxCreator, Doc pdf, string flg, string bDate, DataMitumori.T_MitumoriHeaderDataTable dtH)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (type == "Mitumori")
                {
                    xlsxCreator = StatementsDataSet8(xlsxCreator, iGyousu, dt.Rows[i], iViewCount, iPdfCnt, iPdfCntAll, SouGokei, flg, bDate, dtH, row);
                }
                if (type == "Nouhin")
                {
        ...
        private XlsxCreator StatementsDataSet8(XlsxCreator xlsxCreator, int iGyousu, DataRow dl, int iViewCount, int iPdfCnt, int iPdfCntAll, int souGokei, string flg, string bDate, DataMitumori.T_MitumoriHeaderDataTable dtH, int row)
        {
            DataMitumori.T_MitumoriDataTable dt = new DataMitumori.T_MitumoriDataTable();
            ...
            xlsxCreator.Cell("F" + plus1.ToString()).Value = dr.KeitaiMei;
            xlsxCreator.Cell("G" + plus1.ToString()).Value = dr.JutyuSuryou;
            xlsxCreator.Cell("H" + plus1.ToString()).Value = dr.JutyuTanka.ToString("0,0");
        ...
        private XlsxCreator StatementsDataSet5(XlsxCreator xlsxCreator, int iGyousu, DataRow dr, int iViewCount, int iPdfCnt, int iPdfCntAll, int souGokei, string flg, string bDate, DataMitumori.T_MitumoriHeaderDataTable dtH, int row)
        {
            ...
            if (iGyousu == 1)
            {
                ...
                if (iGyousu == 1)
                {
                    ...
                    if (dl.Zeikubun == "税込")
                    {
                        xlsxCreator.Cell("D9").Value = "￥" + dtH[0].SyohiZeiGokei.ToString("0,0");
                    }
                    else//税抜
                    {
            ...
            {
                xlsxCreator.Cell("H" + (plus1).ToString()).Value = int.Parse(dl.HyojunKakaku).ToString("0,0");
            }
            else
            {
