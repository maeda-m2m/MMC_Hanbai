...
Core.ResizeRadGrid = function(radgrid, arg) {
    Core._ResizeRadGrid(radgrid, 0, true, 0, 0);
};

Core.ResizeRadGridEx = function(radgrid, padding_bottom, bSetWidth, nMaxHeight) {
Core._ResizeRadGrid(radgrid, padding_bottom, bSetWidth, nMaxHeight, 0);
};


Core._ResizeRadGrid = function(radgrid, padding_bottom, bSetWidth, nMaxHeight, call_count) {
    ...
    try {
        ...
        if (document.documentElement.clientWidth < document.documentElement.scrollWidth)
            grid_height_visible = grid_height_visible - 20;   // 縦スクロールバーが表示される場合はその分縮める

        if (bIEOld) grid_height_visible = grid_height_visible - 20;
